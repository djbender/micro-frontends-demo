...
export function validateManifest(json) {
  ...
  if (!json.schemaVersion) return { valid: false, error: 'Missing schemaVersion' };
  if (!Array.isArray(json.mfes)) return { valid: false, error: 'Missing mfes array' };
  for (const entry of json.mfes) {
    for (const field of ['name', 'url', 'module', 'slot', 'route']) {
      if (!entry[field]) return { valid: false, error: `Entry missing field: ${field}` };
    }
